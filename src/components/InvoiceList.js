    ...
    const fetchInvoices = async () => {
      try {
        ...
        const fetchPromises = selectedFacilities.map(async (facility) => {
          try {
            const response = await axios.get(`https://www.biomedwaste.net/api/invoice/${facility}`);
                        ...
                        <TableRow key={invoice._id} hover>
                          ...
                          <TableCell>
                            <IconButton
                              color="success"
                              component="a"
                              href={`https://www.biomedwaste.net/api/invoices/${invoice.fileName}`}
                              download
                              target="_blank"
            ...
            <DialogContent sx={{ display: "flex", justifyContent: "center", alignItems: "center", height: "80vh" }}>
              {currentInvoice && (
                <Worker workerUrl="https://unpkg.com/pdfjs-dist@3.11.174/build/pdf.worker.min.js">
                  <Viewer fileUrl={`https://www.biomedwaste.net/api/invoices/${currentInvoice.fileName}`} />
                </Worker>
              )}
            </DialogContent>
            <DialogActions>
