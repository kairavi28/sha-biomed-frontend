    ...
    const fetchCods = async () => {
      try {
        ...
        const fetchPromises = selectedFacilities.map(async (facility) => {
          try {
            const response = await axios.get(`https://biomedwaste.net/api/cod/${facility}`);
                    ...
                    {(cods[facility] && cods[facility].length > 0) ? (
                      cods[facility].map((cod) => (
                        <TableRow key={cod._id} hover>
                          ...
                          <TableCell>
                            <IconButton color="success" component="a" href={`https://biomedwaste.net/api/cod/${cod.fileName}`} download target="_blank" rel="noopener noreferrer">
                              <DownloadIcon />
                            </IconButton>
          ...
          <DialogContent sx={{ display: "flex", justifyContent: "center", alignItems: "center", height: "80vh" }}>
            {currentCod && (
              <Worker workerUrl="https://unpkg.com/pdfjs-dist@3.11.174/build/pdf.worker.min.js">
                <Viewer fileUrl={`https://biomedwaste.net/api/cod/${currentCod.fileName}`} />
              </Worker>
            )}
          </DialogContent>
          <DialogActions>
