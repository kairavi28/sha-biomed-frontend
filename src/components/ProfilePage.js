...
function ProfilePage() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get(`http://www.biomedwaste.net/api/user/${userId}`);
    ...
    const fetchFacilities = async () => {
      try {
        const response = await axios.get("http://www.biomedwaste.net/api/json/company_name");
  ...
  const handleSave = async () => {
    ...
    try {
      // Send update request
      const response = await axios.put(`http://www.biomedwaste.net/api/user/edit/${userId}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      // Fetch updated user data
      const updatedResponse = await axios.get(`http://www.biomedwaste.net/api/user/${userId}`);
      ...
      if (response.data.newlyAddedFacilities.length > 0) {
        ...
        for (const facilityName of facilityNames) {
          await axios.post(`http://www.biomedwaste.net/api/user/request-facility`, { userId, facilityName }, {
