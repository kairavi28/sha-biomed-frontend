...
function ProfilePage() {
  ...
  useEffect(() => {
    if (userData && issues.length > 0) {
      const userEmail = userData.email;
      const userFacilityNames = (userData.facilities || [])
        .filter(f => f.approved)
        .map(f => f.name);
      const filtered = issues.filter(
        (issue) => issue.email === userEmail || userFacilityNames.includes(issue.facility)
      );
    ...
    <Box sx={{ backgroundColor: "#f5f5f5", minHeight: "100vh" }}>
      <Container maxWidth="lg" sx={{ pt: { xs: 14, md: 16 }, pb: 6 }}>
        ...
        >
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
              <DescriptionIcon sx={{ color: '#0D2477', fontSize: 24 }} />
              <Typography 
                variant="h6" 
                fontWeight="bold" 
                color="#0D2477"
            ...
            <Typography variant="body2" color="text.secondary" mb={3}>
              Here's a list of all product issues you've reported through the Complaint Services page.
            </Typography>
            ...
            <TableContainer sx={{ borderRadius: 1, overflow: 'hidden' }}>
              <Table>
                <TableHead>
                  <TableRow sx={{ backgroundColor: "#0D2477" }}>
                    <TableCell sx={{ color: 'white', fontWeight: 600, py: 2 }}>Issue #</TableCell>
                    <TableCell sx={{ color: 'white', fontWeight: 600, py: 2 }}>Product Type</TableCell>
                    <TableCell sx={{ color: 'white', fontWeight: 600, py: 2 }}>Description</TableCell>
                    <TableCell sx={{ color: 'white', fontWeight: 600, py: 2 }}>Submitted Date</TableCell>
                    <TableCell sx={{ color: 'white', fontWeight: 600, py: 2 }}>Status</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {filteredIssues.length > 0 ? (
                    filteredIssues.map((issue, index) => (
                      ...
                      >
                        <TableCell sx={{ py: 2 }}>{index + 1}</TableCell>
                        <TableCell sx={{ py: 2 }}>{issue.productType || 'N/A'}</TableCell>
                        <TableCell sx={{ py: 2, maxWidth: 250, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {issue.description}
                        </TableCell>
                        <TableCell sx={{ py: 2 }}>
                          {new Date(issue.createdAt || issue.date).toLocaleDateString("en-US", {
                            year: "numeric",
                            month: "numeric",
                            day: "numeric"
                          })}
                        ...
                        <TableCell sx={{ py: 2 }}>
                          <Chip
                            ...
                            size="small"
                            sx={{
                              backgroundColor: issue.status === "Resolved" ? '#4caf50' : issue.status === "Closed" ? '#9e9e9e' : '#D9DE38',
                              color: issue.status === "Resolved" || issue.status === "Closed" ? 'white' : '#333',
                              fontWeight: 500,
                              borderRadius: 1,
                              px: 1
                            }}
