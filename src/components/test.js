...
const InvoiceList = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        const response = await axios.get(`http://biomedwaste.net/api/user/${currentUserId}`);
    ...
    const fetchInvoices = async () => {
      try {
        ...
        const fetchPromises = selectedFacilities.map(async (facility) => {
          try {
            const response = await axios.get(`http://biomedwaste.net/api/invoice/${facility}`);
                    ...
                    {(invoices[facility] && invoices[facility].length > 0) ? (
                      invoices[facility].map((invoice) => (
                        <TableRow key={invoice._id} hover>
                          ...
                          <TableCell>
                            <IconButton color="success" component="a" href={`http://biomedwaste.net/api/invoices/${invoice.fileName}`} download>
                              <DownloadIcon />
                            </IconButton>
                          </TableCell>
          ...
          <DialogContent sx={{ display: "flex", justifyContent: "center", alignItems: "center", height: "80vh" }}>
            {currentInvoice && (
              <Worker workerUrl="https://unpkg.com/pdfjs-dist@3.11.174/build/pdf.worker.min.js">
                <Viewer fileUrl={`http://biomedwaste.net/api/invoices/${currentInvoice.fileName}`} />
              </Worker>
            )}
          </DialogContent>
          <DialogActions>
